...
export default {
  ...
  setup() {
    ...
    const fetchProducts = async () => {
      try {
        loading.value = true
        const response = await product.getProductsByCategory('laptop', {
          params: {
        ...
        if (!response || typeof response !== 'object') {
          throw new Error('Failed to fetch data: The server returned an invalid response')
        }
        const { products: productList, totalPages: total } = response
        if (!Array.isArray(productList)) {
          throw new Error('Failed to fetch data: Incorrect product data format')
        }
        products.value = productList
        totalPages.value = total || 1
      } catch (error) {
        console.error('Failed to load laptop products. Please try again later', error)
      } finally {
        loading.value = false
      }
    }
    ...
    const getPriceRange = (range) => {
      switch (range) {
        ...
        case '6000-8000':
          return { min: 6000, max: 8000 }
        case 'above8000':
          return { min: 8000, max: 300000 }
        default:
          return null
      }
    }
    ...
    const priceOptions = [
      { value: 'all', label: 'All' },
      { value: 'under4000', label: '0 - 4000 AUD' },
      { value: '4000-6000', label: '4000 - 6000 AUD' },
      { value: '6000-8000', label: '6000 - 8000 AUD' },
      { value: 'above8000', label: 'Above 8000 AUD' }
    ]
